...
The name of the secret that holds the credentials for a remote container registry. Default: `gitlab-agent-pull-secret`, which the default `init-image` needs. Whatever you set, the job pod also carries `gitlab-agent-pull-secret` and `nexus-agent-pull-secret`, so images on either registry pull; set it to an empty string to attach no secret at all.
...
Override the [job initContainer](https://kubernetes.io/docs/concepts/workloads/pods/init-containers/). A buildkite-agent binary is expected to exist to do the checkout, along with git and ssh. The default is `registry.gitlab.com/greymatter-io/pipeline-oci/buildkite-agent:latest`, a multi-arch image (the upstream default `embarkstudios/k8s-buildkite-agent` is amd64-only and fails on the ARM builder pool). If set to an empty string no init container is used.
